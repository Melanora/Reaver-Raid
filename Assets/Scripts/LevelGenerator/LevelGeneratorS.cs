...
//    private float timeStart, timeEnd, timeElapsed;
    ...
    public static LevelGeneratorS Generator;
...
//        _transform = transform;
    ...
    public void GenerateLevel(int level, Tilemap tilemap, Transform trans)
    {
//        timeStart = Time.realtimeSinceStartup;
        ...
        if(IsOddLevel(_levelNumber))
            GenerateBlock = GenerateBlockOdd;
        else
            GenerateBlock = GenerateBlockEven;
...
//        GenerateEnemies();
    ...
    {
        if(IsOddLevel(_levelNumber)) return;
...
//                        else spawnOffset -= _tileSize.x;
            ...
            pos.y = block*blockPixelSize -lineInBLock*_tileSize.y +_transform.position.y;// +_tileSize.y/2;
            pos.z = -5;
            ...
            {                    
                ...
                go.GetComponent<RRactor>().Init(_transform, patrolType, activationDist, pos);
                go.SetActive(true);
                enemiesGenerated++;
            }
...
//        Debug.Log($"Level {_levelNumber}. Enemies generated: {enemiesGenerated}");
    ...
    bool IsOddLevel(int x) => ((x%2) != 0);

    bool CheckRandomize() => (_lfsrLevel.low & 48) == 0;

    int GetDirection() => Mathf.Clamp((_lfsrLevel.low & 3)-1 , -1, 1);

    int GetRandomRiverState() => (int)UnityEngine.Random.Range(-1, 2);

    int GetBlock() => Mathf.Clamp((_lfsrLevel.low & 12)>>1, 1, 4);
    ...
    PatrolType GetPatrolType() => (PatrolType)Mathf.Clamp((_lfsrEnemies.hi&48)>>4, 0, 2);
